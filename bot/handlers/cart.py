...
from database.models import Cart, CartItem, Order, OrderItem, Product, ProductVariant, User
...
async def get_cart_items(session, cart_id: int):
    result = await session.execute(
        select(
            ...
            ProductVariant.stock.label("variant_stock"),
        )
...
@router.callback_query(lambda callback: callback.data.startswith("add_variant:"))
async def add_variant_handler(callback: CallbackQuery):
    print("DEBUG ADD_VARIANT =", callback.data)
